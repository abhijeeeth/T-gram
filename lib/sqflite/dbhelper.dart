...
class DbHelper {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    // NOC image documents table
    await db.execute('''
      CREATE TABLE noc_image_documents (
        id INTEGER PRIMARY KEY,
        noc_id INTEGER,
        location_img1 TEXT,
        location_img2 TEXT,
        ...
        image3_log TEXT,
        image4_log TEXT,
        FOREIGN KEY (noc_id) REFERENCES noc_applications (id)
      )
    ''');

    // NOC additional documents table
    await db.execute('''
      CREATE TABLE noc_additional_documents (
        id INTEGER PRIMARY KEY,
        noc_id INTEGER,
        category TEXT,
        document TEXT,
    ...
    // NOC comments and files table
    await db.execute('''
      CREATE TABLE noc_comments_and_files (
        id INTEGER PRIMARY KEY,
        noc_id INTEGER,
        officer TEXT,
        comment TEXT,
  ...
  // NOC Application CRUD operations
  Future<int> insertNocApplication(Map<String, dynamic> application) async {
    Database db = await database;
    return await db.insert('noc_applications', application);
  }
  ...
  Future<int> updateNocApplication(Map<String, dynamic> application) async {
    Database db = await database;
    return await db.update(
      'noc_applications',
      application,
  ...
  Future<int> deleteNocApplication(int id) async {
    Database db = await database;
    await db
        .delete('noc_image_documents', where: 'noc_id = ?', whereArgs: [id]);
    await db.delete('noc_additional_documents',
        where: 'noc_id = ?', whereArgs: [id]);
    await db
        .delete('noc_comments_and_files', where: 'noc_id = ?', whereArgs: [id]);
    return await db
        .delete('noc_applications', where: 'id = ?', whereArgs: [id]);
  }

  Future<int> insertNocCommentsAndFiles(Map<String, dynamic> comment) async {
    Database db = await database;
    if (!comment.containsKey('date')) {
      comment['date'] = DateTime.now().toIso8601String();
    }
    return await db.insert('noc_comments_and_files', comment);
  }

  Future<NOCListViewModel> getNocApplicationWithDetails(int id) async {
    Database db = await database;
    final application = await getNocApplication(id);

    if (application == null) {
      return NOCListViewModel(
          status: 'error', message: 'Application not found');
    }

    final imageDocuments = await db.query(
      'noc_image_documents',
      where: 'noc_id = ?',
      whereArgs: [id],
    );

    final additionalDocuments = await db.query(
      'noc_additional_documents',
      where: 'noc_id = ?',
      whereArgs: [id],
    );

    final divisionComments = await db.query(
      'noc_comments_and_files',
      where: 'noc_id = ? AND type = ?',
      whereArgs: [id, 'division'],
    );

    final clerkComments = await db.query(
      'noc_comments_and_files',
      where: 'noc_id = ? AND type = ?',
      whereArgs: [id, 'clerk'],
    );
    ...
    return NOCListViewModel(
      ...
      message: 'Data fetched successfully',
      data: Data(
        nocApplication: NocApplication.fromJson(application),
        imageDocuments:
            imageDocuments.map((doc) => ImageDocuments.fromJson(doc)).toList(),
        additionalDocuments: additionalDocuments
            .map((doc) => AdditionalDocument.fromJson(doc))
            .toList(),
        divisionCommentsAndFiles: divisionComments
            .map((comment) => DivisionCommentsAndFiles.fromJson(comment))
            .toList(),
        clerkCommentsAndFiles: clerkComments
            .map((comment) => ClerkCommentsAndFiles.fromJson(comment))
            .toList(),
      ),
    );
  }
