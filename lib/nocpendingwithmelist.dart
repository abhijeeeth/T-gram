...
class NocPendingListTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<MainBloc, MainState>(
      listener: (context, state) {
        if (state is NOClistPendingFailed) {
  ...
  Widget _buildBody(BuildContext context, MainState state) {
    if (state is NocPendingLIstloading) {
      return const Center(
        child: Column(
    ...
    final inProcessingList =
        Initializer.nocPendingModel.data?.inProcessingList ?? [];

    if (inProcessingList.isEmpty) {
  ...
  Widget _buildHeader(int count) {
    final areaRangeName = Initializer.nocPendingModel.data?.areaRangeName ?? '';
    final group = Initializer.nocPendingModel.data?.group ?? '';
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    '$count applications in processing',
                    style: TextStyle(
                      fontSize: 14,
        ...
        child: SingleChildScrollView(
          ...
          child: ConstrainedBox(
            ...
            child: DataTable(
              ...
              rows: List.generate(inProcessingList.length, (index) {
                final item = inProcessingList[index];
                return DataRow(
